...
const TOOL_REGISTRY = {
  ...
  "image-generation": {
    ...
    version: "1.0.0",
    handlerId: "image-generation",
    purpose: "Generate images from a prompt via a configured provider (OpenAI/Stability). Fail-closed until a key is set.",
    riskTier: "moderate",
    approvalPolicy: "plan_approval",
    availability: AVAILABILITY.AVAILABLE,
    availabilityState: AVAILABILITY.AVAILABLE,
    featureFlag: "IMAGE_GEN_ENABLED",
    executable: true,
    redactionFields: ["apiKey"],
    inputSchema: {
      type: "object",
      ...
      properties: {
        ...
        provider: { type: "string", enum: ["openai", "stability"] },
  ...
  "browser-operator": {
    ...
    version: "0.0.0",
    handlerId: null,
    purpose: "Blocked. Browser Operator (active-tab, delegation, form filling, data extraction) requires a secure runner decision before it can be enabled.",
    riskTier: "critical",
    approvalPolicy: "just_in_time",
    availability: AVAILABILITY.BLOCKED,
    availabilityState: AVAILABILITY.BLOCKED,
