...
test("dispatcher: safe-internal, web-search, research, code-generation, document-generation, and memory workflows are registered", async () => {
  const dispatcher = require("../server-workflow-dispatch");
  const list = dispatcher.listWorkflows();
  assert.equal(list.length, 6);
  const ids = list.map((w) => w.id).sort();
  assert.deepEqual(ids, ["code-generation", "document-generation", "memory", "research", "safe-internal", "web-search"]);
