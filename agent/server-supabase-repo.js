...
function mapTask(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, projectId: r.project_id, creatorId: r.creator_id,
    title: r.title, desiredOutcome: r.desired_outcome, instructions: r.instructions,
    status: r.status, priority: r.priority, activePlanVersion: r.active_plan_version,
    budgetLimit: r.budget_limit != null ? Number(r.budget_limit) : null,
    timeLimitSeconds: r.time_limit_seconds, cancelledAt: r.cancelled_at,
    cancellationReason: r.cancellation_reason, createdAt: r.created_at, updatedAt: r.updated_at,
  };
}
function mapPlan(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, taskId: r.task_id, version: r.version,
    status: r.status, author: r.author, summary: r.summary,
    assumptions: r.assumptions || [], risks: r.risks || [],
...
function mapPlanStep(r) {
  if (!r) return null;
  return {
    id: r.id, planId: r.plan_id, taskId: r.task_id, workspaceId: r.workspace_id,
    position: r.position, title: r.title, purpose: r.purpose,
    dependencies: r.dependencies || [], toolClass: r.tool_class,
    inputSummary: r.input_summary, expectedOutput: r.expected_output,
    riskTier: r.risk_tier, requiresApproval: r.requires_approval, status: r.status,
    estimatedDuration: r.estimated_duration,
    estimatedCost: r.estimated_cost != null ? Number(r.estimated_cost) : null,
    retryPolicy: r.retry_policy || { maxRetries: 0 }, createdAt: r.created_at,
  };
}
function mapRun(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, taskId: r.task_id, planId: r.plan_id,
    status: r.status, attemptCount: r.attempt_count, parentRunId: r.parent_run_id,
    provider: r.provider, toolSummary: r.tool_summary, startedAt: r.started_at,
    endedAt: r.ended_at, failureClassification: r.failure_classification,
...
function mapRunEvent(r) {
  if (!r) return null;
  return {
    id: r.id, runId: r.run_id, taskId: r.task_id, workspaceId: r.workspace_id,
    sequence: r.sequence, event: r.event, summary: r.summary,
    metadata: r.metadata || {}, actor: r.actor, createdAt: r.created_at,
  };
}
function mapApproval(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, projectId: r.project_id, taskId: r.task_id,
    runId: r.run_id, planStepId: r.plan_step_id, action: r.action_type,
    actionTarget: r.action_target, payloadHash: r.payload_hash,
    payloadPreview: r.payload_preview || {}, riskTier: r.risk_tier, reason: r.reason,
    requestedBy: r.requested_by, status: r.status, expiresAt: r.expires_at,
    decidedAt: r.decided_at, decidedBy: r.decided_by, decisionNote: r.decision_note,
    executedResult: r.executed_result, createdAt: r.created_at,
  };
}
function mapArtifact(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, taskId: r.task_id, runId: r.run_id,
    type: r.type, displayName: r.display_name, mediaType: r.media_type,
    storageRef: r.storage_ref, contentHash: r.content_hash,
    sizeBytes: r.size_bytes != null ? Number(r.size_bytes) : 0, state: r.state,
    summary: r.summary, provenance: r.provenance || {}, createdBy: r.created_by,
    createdAt: r.created_at,
  };
}
function mapAuditEvent(r) {
  if (!r) return null;
  return {
    id: r.id, workspaceId: r.workspace_id, actorId: r.actor_id, actorType: r.actor_type,
    action: r.action, objectType: r.object_type, objectId: r.object_id,
    metadata: r.metadata || {}, correlationId: r.correlation_id, createdAt: r.created_at,
  };
}
...
async function getTask(client, workspaceId, taskId) {
  ...
  const { data, error } = await client.from("tasks")
    .select("*").eq("workspace_id", workspaceId).eq("id", taskId).maybeSingle();
  if (error) return { ok: false, message: error.message };
...
async function updateTask(client, workspaceId, taskId, updates) {
  if (!client) return NOT_CONFIGURED;
  const WHITELIST = {
    title: "title", desired_outcome: "desiredOutcome", instructions: "instructions",
    status: "status", priority: "priority", active_plan_version: "activePlanVersion",
    budget_limit: "budgetLimit", time_limit_seconds: "timeLimitSeconds",
  ...
  const { data, error } = await client.from("tasks")
    .update(filtered).eq("workspace_id", workspaceId).eq("id", taskId).select().single();
  if (error) return { ok: false, message: error.message };
  return { ok: true, task: mapTask(data) };
}
...
async function nextPlanVersion(client, workspaceId, taskId) {
  ...
  const { data, error } = await client.from("plans")
    .select("version").eq("workspace_id", workspaceId).eq("task_id", taskId)
...
async function createPlan(client, p) {
  ...
  const { data, error } = await client.from("plans").insert({
    workspace_id: p.workspaceId, task_id: p.taskId, version: p.version || 1,
    status: "draft", author: p.author || "system",
    summary: p.summary || "", assumptions: p.assumptions || [], risks: p.risks || [],
  }).select().single();
  if (error) return { ok: false, message: error.message };
...
async function createPlanSteps(client, steps) {
  ...
  const rows = steps.map((s) => ({
    workspace_id: s.workspaceId, plan_id: s.planId, task_id: s.taskId,
    ...
    risk_tier: ["low", "moderate", "high", "critical"].includes(s.riskTier) ? s.riskTier : "low",
...
async function listPlansForTask(client, workspaceId, taskId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("plans")
    .select("*").eq("workspace_id", workspaceId).eq("task_id", taskId)
    .order("version", { ascending: true });
  if (error) return { ok: false, message: error.message };
  const planIds = (data || []).map((p) => p.id);
  let steps = [];
  if (planIds.length > 0) {
    const { data: stepData, error: stepErr } = await client.from("plan_steps")
      .select("*").in("plan_id", planIds).order("position", { ascending: true });
...
async function getPlan(client, workspaceId, planId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("plans")
    .select("*").eq("workspace_id", workspaceId).eq("id", planId).maybeSingle();
  if (error) return { ok: false, message: error.message };
  if (!data) return { ok: false, message: "Plan not found" };
  const { data: stepData, error: stepErr } = await client.from("plan_steps")
    .select("*").eq("plan_id", planId).order("position", { ascending: true });
  if (stepErr) return { ok: false, message: stepErr.message };
  return { ok: true, plan: { ...mapPlan(data), steps: (stepData || []).map(mapPlanStep) } };
}
...
async function updatePlanStatus(client, workspaceId, planId, newStatus) {
  ...
  const { data, error } = await client.from("plans")
    .update(updates).eq("workspace_id", workspaceId).eq("id", planId).select().single();
  if (error) return { ok: false, message: error.message };
  if (newStatus === "approved") {
    await client.from("plans").update({ status: "superseded" })
      .eq("workspace_id", workspaceId).eq("task_id", data.task_id)
      .eq("status", "approved").neq("id", planId);
  }
  return { ok: true, plan: mapPlan(data) };
}
...
async function createRun(client, r) {
  ...
  const { data, error } = await client.from("runs").insert({
    workspace_id: r.workspaceId, task_id: r.taskId, plan_id: r.planId || null,
    status: "queued", attempt_count: r.attemptCount || 1,
...
async function updateRunStatus(client, runId, status, details = {}) {
  ...
  if (["succeeded", "failed", "cancelled", "timed_out"].includes(status)) {
    ...
    if (details.failureClassification) updates.failure_classification = details.failureClassification;
  ...
  const { data, error } = await client.from("runs")
    .update(updates).eq("id", runId).select().single();
  if (error) return { ok: false, message: error.message };
  return { ok: true, run: mapRun(data) };
}
...
async function nextRunEventSequence(client, runId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("run_events")
    .select("sequence").eq("run_id", runId)
...
async function createRunEvent(client, e) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("run_events").insert({
    workspace_id: e.workspaceId, run_id: e.runId, task_id: e.taskId || null,
    sequence: Number(e.sequence) || 0, event: e.event,
    summary: String(e.summary || "").slice(0, 500),
    metadata: e.metadata || {}, actor: e.actor || "system",
  }).select().single();
  if (error) return { ok: false, message: error.message };
...
async function listRunEvents(client, runId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("run_events")
    .select("*").eq("run_id", runId).order("sequence", { ascending: true
  });
  if (error) return { ok: false, message: error.message };
...
async function findActiveRun(client, workspaceId, taskId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("runs")
    .select("id, status").eq("workspace_id", workspaceId).eq("task_id", taskId)
    .in("status", ["queued", "active"]).limit(1);
  if (error) return { ok: false, message: error.message };
...
async function createApproval(client, a) {
  ...
  const { data, error } = await client.from("approval_requests").insert({
    ...
    status: "pending",
    expires_at: a.expiresAt || new Date(Date.now() + 15 * 60 * 1000).toISOString(),
  }).select().single();
  if (error) return { ok: false, message: error.message };
...
async function getApproval(client, workspaceId, approvalId) {
  ...
  const { data, error } = await client.from("approval_requests")
    .select("*").eq("workspace_id", workspaceId).eq("id", approvalId).maybeSingle();
  if (error) return { ok: false, message: error.message };
...
async function updateApprovalStatus(client, workspaceId, approvalId, fields) {
  if (!client) return NOT_CONFIGURED;
  const WL = {
    status: "status", decided_at: "decidedAt", decided_by: "decidedBy",
    decision_note: "decisionNote", executed_result: "executedResult",
  };
  const filtered = {};
  ...
  const { data, error } = await client.from("approval_requests")
    .update(filtered).eq("workspace_id", workspaceId).eq("id", approvalId).select().single();
  if (error) return { ok: false, message: error.message };
  return { ok: true, approval: mapApproval(data) };
}
...
async function createArtifact(client, a) {
  ...
  const { data, error } = await client.from("artifacts").insert({
    workspace_id: a.workspaceId,
    task_id: a.taskId || null, run_id: a.runId || null,
    type: a.type || "document", display_name: a.displayName,
    media_type: a.mediaType || "text/markdown",
    storage_ref: a.storageRef || "", content_hash: a.contentHash || "",
    size_bytes: a.sizeBytes || 0, state: a.state || "draft",
    summary: a.summary || "", provenance: a.provenance || {},
...
async function updateArtifactState(client, workspaceId, artifactId, state) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("artifacts")
    .update({ state }).eq("workspace_id", workspaceId).eq("id", artifactId).select().single();
  if (error) return { ok: false, message: error.message };
  return { ok: true, artifact: mapArtifact(data) };
}
...
async function getArtifact(client, workspaceId, artifactId) {
  if (!client) return NOT_CONFIGURED;
  const { data, error } = await client.from("artifacts")
    .select("*").eq("workspace_id", workspaceId).eq("id", artifactId).maybeSingle();
  if (error) return { ok: false, message: error.message };
...
async function listArtifacts(client, workspaceId, filters) {
  filters = filters || {};
  if (!client) return NOT_CONFIGURED;
  let q = client.from("artifacts").select("*")
    .eq("workspace_id", workspaceId).order("created_at", { ascending: false });
...
module.exports = {
  mapTask, mapPlan, mapPlanStep, mapRun, mapRunEvent,
  mapApproval, mapArtifact, mapAuditEvent,
  createTask, getTask, listTasks, updateTask,
  nextPlanVersion, createPlan, createPlanSteps,
  listPlansForTask, getPlan, updatePlanStatus,
  createRun, updateRunStatus, getRun, listRunsForTask,
  nextRunEventSequence, createRunEvent, listRunEvents, findActiveRun,
  createApproval, getApproval, listApprovals, updateApprovalStatus,
  createArtifact, updateArtifactState, getArtifact, listArtifacts,
