...
async function createTask(req, res) {
  ...
  if (title.length > 200) return json(res, 400, { ok: false, message: "title exceeds 200 chars" });
  ...
  const timeLimitSeconds = body.timeLimitSeconds != null ? Number(body.timeLimitSeconds) : null;
...
async function updateTask(req, res, taskId) {
  ...
  if (body.status) {
    ...
    try {
      ...
      return json(res, 409, { ok: false, message: err.message, code: "invalid_transition" });
    }
  }
  const result = await supabaseRepo.updateTaskInDb(body.workspaceId, taskId, body);
  if (!result.ok) return json(res, 500, { ok: false, message: result.message });
  return json(res, 200, { ok: true, task: result.task });
}
...
async function cancelTask(req, res, taskId) {
  ...
  const result = await supabaseRepo.updateTaskInDb(body.workspaceId, taskId, {
    ...
    cancellationReason: String(body.reason || "Cancelled by user").slice(0, 500),
  });
  if (!result.ok) return json(res, 500, { ok: false, message: result.message });
  return json(res, 200, { ok: true, task: result.task });
}
...
async function generatePlan(req, res) {
  ...
  const taskResult = await supabaseRepo.getTaskInDb(body.workspaceId, body.taskId);
  if (!taskResult.ok) return json(res, 404, { ok: false, message: taskResult.message });
  const task = taskResult.task;

  const draft = planner.generatePlanDraft(task);
  if (!draft.ok) return json(res, 400, { ok: false, message: draft.message });

  const nextVer = await supabaseRepo.nextPlanVersion(body.workspaceId, body.taskId);
  if (!nextVer.ok) return json(res, 500, { ok: false, message: nextVer.message });
  ...
  if (task.status === "draft") {
    try {
      ...
      await supabaseRepo.updateTaskInDb(body.workspaceId, task.id, { status: "planning" });
    } catch { /* ignore state errors here */ }
  }
  ...
  if (!planResult.ok) return json(res, 500, { ok: false, message: planResult.message });
  ...
  if (!stepsResult.ok) return json(res, 500, { ok: false, message: stepsResult.message });

  // After draft creation, transition task to awaiting_plan_review
  try {
    sm.assertTransition("task", "planning", "awaiting_plan_review", auth.user.id);
    await supabaseRepo.updateTaskInDb(body.workspaceId, task.id, { status: "awaiting_plan_review" });
  } catch { /* tolerate */ }
...
async function listPlans(req, res, url) {
  ...
  const auth = await requireWorkspace(req, workspaceId);
  if (!auth.ok) return json(res, auth.status, { ok: false, message: auth.error });
  if (!taskId) return json(res, 400, { ok: false, message: "taskId is required" });
...
async function revisePlan(req, res) {
  ...
  const planResult = await supabaseRepo.getPlanInDb(body.workspaceId, body.planId);
  if (!planResult.ok) return json(res, 404, { ok: false, message: planResult.message });
  ...
  const task = (await supabaseRepo.getTaskInDb(body.workspaceId, prev.taskId)).task;
  let base = planner.generatePlanDraft(task);
  if (!base.ok) return json(res, 400, { ok: false, message: base.message });

  const overrides = body.overrides || {};
  const summary = String(overrides.summary || base.planDraft.summary).slice(0, 2000);
  const assumptions = Array.isArray(overrides.assumptions) ? overrides.assumptions : base.planDraft.assumptions;
  const risks = Array.isArray(overrides.risks) ? overrides.risks : base.planDraft.risks;
  const steps = Array.isArray(overrides.steps) && overrides.steps.length > 0
    ? overrides.steps.map((s, i) => ({
        ...
        riskTier: ["low", "moderate", "high", "critical"].includes(s.riskTier) ? s.riskTier : "low",
        requiresApproval: Boolean(s.requiresApproval),
        estimatedDuration: s.estimatedDuration != null ? Number(s.estimatedDuration) : null,
  ...
  const nextVer = await supabaseRepo.nextPlanVersion(body.workspaceId, prev.taskId);
  if (!nextVer.ok) return json(res, 500, { ok: false, message: nextVer.message });
  ...
  if (!newPlan.ok) return json(res, 500, { ok: false, message: newPlan.message });
  const stepRows = steps.map((s) => ({ ...s, workspaceId: body.workspaceId, planId: newPlan.plan.id, taskId: prev.taskId }));
  const stepInsert = await supabaseRepo.createPlanStepsInDb(stepRows);
  if (!stepInsert.ok) return json(res, 500, { ok: false, message: stepInsert.message });
  ...
  const approval = await supabaseRepo.createApprovalInDb({
    ...
    payloadHash,
    payloadPreview: { planId: newPlan.plan.id, planVersion: newPlan.plan.version, summary, feedback: String(body.feedback || "").slice(0, 1000) },
    riskTier: "moderate",
    reason: `Plan v${newPlan.plan.version} awaiting review.`,
    requestedBy: auth.user.id,
  });
  if (!approval.ok) return json(res, 500, { ok: false, message: approval.message });
  ...
  await supabaseRepo.createAuditEventInDb({
    ...
    metadata: { previousPlanId: prev.id, previousVersion: prev.version, version: newPlan.plan.version, payloadHash, feedback: String(body.feedback || "").slice(0, 500) },
  });
...
async function decidePlan(req, res) {
  ...
  if (!["approve", "reject"].includes(decision)) {
    return json(res, 400, { ok: false, message: "decision must be 'approve' or 'reject'" });
  }
  ...
  try {
    sm.assertTransition("plan", plan.status, decision === "approve" ? "approved" : "rejected", auth.user.id);
  } catch (err) {
    return json(res, 409, { ok: false, message: err.message, code: "invalid_transition" });
  }

  const approvals = await supabaseRepo.listApprovalsInDb(body.workspaceId, "pending");
  const planApprovals = (approvals.approvals || []).filter((a) => a.actionTarget === plan.id);
  const latestApproval = planApprovals.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
  if (latestApproval && body.payloadHash && latestApproval.payloadHash !== body.payloadHash) {
    return json(res, 409, {
      ok: false,
      message: "Plan approval payload hash mismatch. Reload the latest plan version.",
      code: "stale_approval",
    });
  }

  const update = await supabaseRepo.updatePlanStatusInDb(body.workspaceId, plan.id, decision === "approve" ? "approved" : "rejected");
  if (!update.ok) return json(res, 500, { ok: false, message: update.message });

  if (latestApproval) {
    await supabaseRepo.updateApprovalStatusInDb(body.workspaceId, latestApproval.id, {
  ...
  if (decision === "approved") {
    try {
      sm.assertTransition("task", "awaiting_plan_review", "queued", auth.user.id);
      await supabaseRepo.updateTaskInDb(body.workspaceId, plan.taskId, { status: "queued", activePlanVersion: plan.version });
    } catch { /* tolerate */ }
  } else if (decision === "rejected") {
    try {
      sm.assertTransition("task", "awaiting_plan_review", "planning", auth.user.id);
      await supabaseRepo.updateTaskInDb(body.workspaceId, plan.taskId, { status: "planning" });
    } catch { /* tolerate */ }
  }
  ...
  await supabaseRepo.createAuditEventInDb({
    ...
    objectType: "plan",
    objectId: plan.id,
    metadata: { taskId: plan.taskId, version: plan.version, payloadHash: body.payloadHash || null, note: String(body.note || "").slice(0, 500) },
  });
...
async function startRun(req, res) {
  ...
  const approvals = await supabaseRepo.listApprovalsInDb(body.workspaceId, "approved");
  ...
  if (!result.ok) {
    return json(res, result.failureClassification === "policy_blocked" ? 403 : 409, {
      ok: false,
...
async function retryRun(req, res) {
  ...
  const taskResult = await supabaseRepo.getTaskInDb(body.workspaceId, orig.run.taskId);
  if (!taskResult.ok) return json(res, 404, { ok: false, message: taskResult.message });
  const task = taskResult.task;
  if (orig.run.planId) {
    const planResult = await supabaseRepo.getPlanInDb(body.workspaceId, orig.run.planId);
    if (planResult.ok) {
      const approvals = await supabaseRepo.listApprovalsInDb(body.workspaceId, "approved");
      const planApproval = (approvals.approvals || []).find(
        (a) => a.actionTarget === planResult.plan.id && a.actionType === "plan.approve"
      );
      const result = await workflow.dispatch("safe-internal", {
        supabase: supabaseRepo,
        task,
  ...
  return json(res, 400, { ok: false, message: "Retry requires the original plan to still exist." });
}
...
async function artifactDownloadUrl(req, res) {
  ...
  const auth = await requireWorkspace(req, body.workspaceId);
  if (!auth.ok) return json(res, auth.status, { ok: false, message: auth.error });
  const result = await supabaseRepo.getArtifactInDb(body.workspaceId, body.artifactId);
  if (!result.ok) return json(res, 404, { ok: false, message: result.message });
  const filename = (body.filename && String(body.filename).replace(/[^a-zA-Z0-9._-]/g, "_")) || "artifact.md";
  const urlResult = await supabaseRepo.getArtifactSignedUrl(body.workspaceId, body.artifactId, filename, body.expiresIn || 3600);
  if (!urlResult.ok) return json(res, 500, { ok: false, message: urlResult.message });
  return json(res, 200, { ok: true, url: urlResult.url, expiresIn: body.expiresIn || 3600 });
}
...
async function handleDurable(req, res, url) {
  ...
  // Tasks
  if (path === "/api/durable/tasks" && method === "GET") return listTasks(req, res, url);
  if (path === "/api/durable/tasks" && method === "POST") return createTask(req, res);
  const taskMatch = path.match(/^\/api\/durable\/tasks\/([^\/]+)$/);
  if (taskMatch) {
    if (method === "GET") return getTask(req, res, url, taskMatch[1]);
    if (method === "PATCH" || method === "PUT") return updateTask(req, res, taskMatch[1]);
  }
  ...
  // Plans
  if (path === "/api/durable/plans/generate" && method === "POST") return generatePlan(req, res);
  if (path === "/api/durable/plans/revise" && method === "POST") return revisePlan(req, res);
  if (path === "/api/durable/plans/decide" && method === "POST") return decidePlan(req, res);
  if (path === "/api/durable/plans" && method === "GET") return listPlans(req, res, url);
  const planMatch = path.match(/^\/api\/durable\/plans\/([^\/]+)$/);
  if (planMatch && method === "GET") return getPlan(req, res, url, planMatch[1]);

  // Approvals
  if (path === "/api/durable/approvals" && method === "GET") return listApprovals(req, res, url);
  const approvalMatch = path.match(/^\/api\/durable\/approvals\/([^\/]+)$/);
  if (approvalMatch && method === "GET") return getApproval(req, res, url, approvalMatch[1]);

  // Runs
  if (path === "/api/durable/runs/start" && method === "POST") return startRun(req, res);
  if (path === "/api/durable/runs/cancel" && method === "POST") return cancelRun(req, res);
  if (path === "/api/durable/runs/retry" && method === "POST") return retryRun(req, res);
  if (path === "/api/durable/runs" && method === "GET") return listRuns(req, res, url);
  ...
  if (runEventsMatch && method === "GET") return listRunEvents(req, res, url, runEventsMatch[1]);

  // Artifacts
  if (path === "/api/durable/artifacts" && method === "GET") return listArtifacts(req, res, url);
  if (path === "/api/durable/artifacts/download-url" && method === "POST") return artifactDownloadUrl(req, res);
  const artifactMatch = path.match(/^\/api\/durable\/artifacts\/([^\/]+)$/);
  if (artifactMatch && method === "GET") return getArtifact(req, res, url, artifactMatch[1]);

  // Audit
  if (path === "/api/durable/audit" && method === "GET") return listAuditEvents(req, res, url);
...
module.exports = { handleDurable, listTasks, createTask, getTask, updateTask, cancelTask,
  generatePlan, revisePlan, decidePlan, listPlans, getPlan,
  listApprovals, getApproval, startRun, listRuns, getRun, listRunEvents, cancelRun, retryRun,
  listArtifacts, getArtifact, artifactDownloadUrl, listAuditEvents, listWorkflows,
};
